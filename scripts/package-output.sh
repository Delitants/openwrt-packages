...
runtime_apk=$(find_one_apk runtime 'netwatch-1.1.0-r2.apk')
luci_apk=$(find_one_apk LuCI 'luci-app-netwatch-1.1.0-r2.apk')
scheduled_backup_apk=$(find_one_apk 'Scheduled Backup LuCI' \
	'luci-app-scheduled-backup-1.0.0-r3.apk')

mkdir -p "$output_dir"
runtime_output=$output_dir/netwatch_1.1.0-r2_all.apk
luci_output=$output_dir/luci-app-netwatch_1.1.0-r2_all.apk
...
rm -f "$runtime_output" "$luci_output" "$scheduled_backup_output" \
	"$source_output" "$checksums"
cp "$runtime_apk" "$runtime_output"
cp "$luci_apk" "$luci_output"
cp "$scheduled_backup_apk" "$scheduled_backup_output"
