...
printf 'runtime apk fixture\n' > "$tmp/work/sdk/bin/packages/base/netwatch-1.1.0-r6.apk"
printf 'luci apk fixture\n' > "$tmp/work/sdk/bin/packages/base/luci-app-netwatch-1.1.0-r6.apk"
...
rm "$tmp/feed/x86_64/netwatch-1.1.0-r2.apk" \
	"$tmp/feed/x86_64/luci-app-netwatch-1.1.0-r2.apk"
printf 'runtime signed r6 feed fixture\n' > \
	"$tmp/feed/x86_64/netwatch-1.1.0-r6.apk"
printf 'luci signed r6 feed fixture\n' > \
	"$tmp/feed/x86_64/luci-app-netwatch-1.1.0-r6.apk"
git --git-dir="$tmp/work/git-metadata" --work-tree="$tmp" \
	add -u feed/x86_64
git --git-dir="$tmp/work/git-metadata" --work-tree="$tmp" \
	add feed/x86_64/netwatch-1.1.0-r6.apk \
	feed/x86_64/luci-app-netwatch-1.1.0-r6.apk
git --git-dir="$tmp/work/git-metadata" --work-tree="$tmp" \
	commit -q -m 'current r6 fixture source'
...
for artifact in \
	netwatch_1.1.0-r6_all.apk \
	luci-app-netwatch_1.1.0-r6_all.apk \
