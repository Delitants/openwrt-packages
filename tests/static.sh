...
do
	for declaration in 'PKG_VERSION:=1.1.0' 'PKG_RELEASE:=4'; do
...
for expectation in \
	'runtime=outputs/netwatch_1.1.0-r4_all.apk' \
	'luci=outputs/luci-app-netwatch_1.1.0-r4_all.apk' \
	'source_archive=outputs/openwrt-netwatch-1.1.0-source.tar.gz' \
	'.info.version == "1.1.0-r4"' \
...
if (!build.includes('The 1.1.0-r4 release artifacts use the stable output names above.') ||
	!build.includes('The signed repository feed for this release contains `netwatch-1.1.0-r4` and `luci-app-netwatch-1.1.0-r4`.') ||
	!build.includes('This source document does not attest to public-byte equality or live-router verification.'))
	errors.push('README must describe the signed local r4 release without temporal publication claims');

if (build.includes('The source tree is prepared for the 1.1.0-r4 release outputs.') ||
	build.includes('After a later build, signing, and feed publication'))
	errors.push('README must not retain pre-publication future-tense r4 wording');
