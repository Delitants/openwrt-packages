...
The 1.1.0-r5 release artifacts use the stable output names above. The signed
repository feed for this release contains `netwatch-1.1.0-r5` and
`luci-app-netwatch-1.1.0-r5`. This source document does not attest to
...
./scripts/in-sdk.sh /sdk/staging_dir/host/bin/apk adbdump --format json \
  /src/outputs/netwatch_1.1.0-r5_all.apk | jq .info
./scripts/in-sdk.sh /sdk/staging_dir/host/bin/apk adbdump --format json \
  /src/outputs/luci-app-netwatch_1.1.0-r5_all.apk | jq .info
...
two `1.1.0-r5` `noarch` manifests. The runtime manifest contains the CA bundle,
